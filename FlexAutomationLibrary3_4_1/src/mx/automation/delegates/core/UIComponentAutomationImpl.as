...
obj.addEventListener(MouseEvent.MOUSE_OVER,monkeyMouseOverHandler,false,EventPriority.DEFAULT+100000,true);
obj.addEventListener(MouseEvent.MOUSE_OUT,monkeyMouseOutHandler,false,EventPriority.DEFAULT+100000,true);
obj.addEventListener(MouseEvent.MOUSE_DOWN,monkeyMouseDownHandler,false,EventPriority.DEFAULT+100001,true);

obj.addEventListener(MouseEvent.MOUSE_UP,monkeyMousePropagationHandler,false,EventPriority.DEFAULT+100000,true);
obj.addEventListener(MouseEvent.ROLL_OVER,monkeyMousePropagationHandler,false,EventPriority.DEFAULT+100000,true);
obj.addEventListener(MouseEvent.ROLL_OUT,monkeyMousePropagationHandler,false,EventPriority.DEFAULT+100000,true);
...
private function monkeyMouseOverHandler(event:MouseEvent):void{
	...
	if(MonkeyAutomationState.monkeyAutomationState.state == MonkeyAutomationState.SNAPSHOT){	
		...
		var isComposite:Boolean = helper.isAutomationComposite(IAutomationObject(pointSource));				
		if(isComposite){
			pointSource = DisplayObject(helper.getAutomationComposite(IAutomationObject(pointSource)));
		}	
		var globalPoint:Point = pointSource.localToGlobal(new Point(0,0));
		var overlayPoint:Point = MonkeyAutomationState.monkeyAutomationState.conversionPlatform.globalToLocal(globalPoint);; 
		MonkeyAutomationState.monkeyAutomationState.graphics.clear();
   		MonkeyAutomationState.monkeyAutomationState.graphics.lineStyle(2,0xff0000,1.0);
   		MonkeyAutomationState.monkeyAutomationState.graphics.drawRect(overlayPoint.x,overlayPoint.y,pointSource.width,pointSource.height);
   		MonkeyAutomationState.monkeyAutomationState.graphics.lineStyle();
		event.stopImmediatePropagation();
	}	
} 
...
//trace("Checking MonkeyAutomationState");	
	if(MonkeyAutomationState.monkeyAutomationState.state == MonkeyAutomationState.SNAPSHOT){
		MonkeyAutomationState.monkeyAutomationState.graphics.clear();
//trace("Cleared graphics in UIComp delegate");		
		event.stopImmediatePropagation();
	}	
} 
...
private function monkeyMouseClickHandler(event:MouseEvent):void{
	if(MonkeyAutomationState.monkeyAutomationState.state == MonkeyAutomationState.SNAPSHOT){		
//		MonkeyAutomationState.monkeyAutomationState.graphics.clear();
//trace("Running Delegate Snapshot MouseCLick handler");
 
        var am:IAutomationManager = Automation.automationManager;
		var helper:IAutomationObjectHelper = Automation.automationObjectHelper;
		
		var isComposite:Boolean = helper.isAutomationComposite(IAutomationObject(uiComponent));			
		if(isComposite){
			var parent:IAutomationObject = helper.getAutomationComposite(IAutomationObject(uiComponent));
			UIComponent(parent).dispatchEvent(new MouseEvent(MouseEvent.CLICK,false));
			return;	
		}
        if(am && am.recording)
        {
